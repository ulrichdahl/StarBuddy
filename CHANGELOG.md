...
- **Signatures are read on every ship's HUD, not one.** The pin beside the number is drawn differently by each ship and the reader was told to expect the one shape it had been shown; across twenty-five screenshots from a MOLE, a Pisces and a Starrunner, twenty of the signatures on screen went unread for looking slightly wrong. What tells a badge from a bright speck is the number beside it, which is now what decides.
